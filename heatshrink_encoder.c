...
/*
 *@brief 动态分配编码器内存
 *@param window_sz2 窗口大小
 *@param lookahead_sz2 预读大小
 */
...
void heatshrink_encoder_reset(heatshrink_encoder *hse) {
  size_t buf_sz = (2 << HEATSHRINK_ENCODER_WINDOW_BITS(hse)); // 获取缓冲区大小
  memset(hse->buffer, 0, buf_sz);                             // 清空缓冲区
  hse->input_size = 0;
...
/*
 *@brief 判断是否处于结束状态, 1表示是结束状态,0表示不是结束状态
 *@param hse 编码器
 */
static int is_finishing(heatshrink_encoder *hse) {
